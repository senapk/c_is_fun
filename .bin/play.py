...
class Colored:
  ...
  def paint(text: str, color: Color, color2: Optional[Color] = None) -> str:
    if not Colored.enabled:
      return text
    return (Colored.__map[color] +
            ("" if color2 is None else Colored.__map[color2]) + text +
            Colored.__map[Color.RESET])
  ...
  def center(text: str, width: int, filler: str) -> str:
    return filler * ((width - Colored.len(text)) // 2) + text + filler * (
        (width - Colored.len(text) + 1) // 2)
...
class Play:
  ...
  def print_quest(self, entry, q, max_title, term_size):
    ...
    if done == size:
      ...
      resume = Colored.red(text)
    else:
      resume = Colored.yellow(text)
    entry = Colored.blue(entry) if q.type == "main" else Colored.magenta(entry)
    tit = q.title.ljust(max_title)
    qlink = ""
    if self.show_url:
      if term_size > self.term_limit:
        qlink = " " + Colored.cyan(q.mdlink)
      else:
        qlink = "\n      " + Colored.cyan(q.mdlink)
    if not self.show_all and done == size:
      return
    print(f"{opening} {entry}{space}{resume} {tit}{qlink}")
  ...
  def show_tasks(self):
    ...
    keys = sorted(single) + sorted(double)
    for entry in keys:
      q = self.quests[entry]
      ...
      for t in q.tasks:
        ...
        if self.show_url:
          if t.key in t.title:
            vlink = Colored.red(t.link)
          else:
            vlink = Colored.yellow(t.link)
          if term_size > self.term_limit:
            vlink = " " + vlink
          else:
            vlink = "\n      " + vlink
  ...
  def expand_range(self, actions):
    ...
    for t in actions:
      ...
      if start_number is not None and end_number is not None:
        ...
        expand += [
            self.calc_letter(i) for i in range(start_index, end_index + 1)
        ]
      else:
        expand.append(t)
    return expand
  ...
  def show_help(self):
    print(
        "Digite os números ou intervalo das tarefas para (marcar/desmarcar), exemplo:"
    )
    print(Colored.green("$ ") + "1 2 5-6")
    print(
        "Digite as letras ou intervalo das quests para (expandir/colapsar), exemplo:"
    )
  ...
  def play(self):
    while True:
      subprocess.run("clear")
      vnum = Colored.green("números") + "(marcar)"
      vlet = Colored.green("letras") + "(expandir)"
      vfold = Colored.green("<") + " ou " + Colored.green(
          ">") + "(expandir todas)"
      vall = Colored.cyan(":a") + Colored.green("ll") + ("[x]" if self.show_all
                                                         else "[ ]")
      vurl = Colored.cyan(":u") + Colored.green("rl") + ("[x]" if self.show_url
                                                         else "[ ]")
      vhelp = Colored.cyan(":h") + Colored.green("elp")
      vclose = Colored.cyan(":q") + Colored.green("uit")
      print(
          f"Digite: {vnum}, {vlet}, {vfold}, {vall}, {vurl}, {vhelp} ou {vclose}."
      )
      self.show_tasks()
      print("\n" + Colored.green("$") + " ", end="")
      actions = input().split(" ")
      if ":q" in actions or ":quit" in actions:
        break
...
def main():
  ...
  if args.quests:
    ...
    print(
        f"Quests alcançáveis: {[str(q.key) for q in reachable_quests if not q.is_complete()]}"
    )
    return
